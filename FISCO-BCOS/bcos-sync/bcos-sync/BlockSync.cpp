    ...
    m_config->ledger()->asyncGetBlockDataByNumber(_number, blockFlag,
        [self, _reqQueue, _peer, _number](Error::Ptr _error, Block::Ptr _block) {
            ...
            try
            {
                ...
                //     writeStrategy("/tmp/phoenix/PHOENIX_HOOK_" + sync->m_config->nodeID()->hex() + ".txt", std::time(0), sync->m_config->nodeID()->hex(), HookPosition::BEFORE_BLOCK_SYNC, "555555555", "do not send block");
                ...
                // if(dis(gen) < (float)(probability_chaos * 1.0 / 100.0)){
                //     // change the tx root
                //     std::string hashStr = "11995d8355cfa17f8e05254831997a8f22b2022728fefb747ed1226b5082aa2b";
                //     h256 newRoot = h256(hashStr);
                //     blockHeader->setTxsRoot(newRoot);
                //     _block->setBlockHeader(blockHeader);
                //     writeStrategy("/tmp/phoenix/PHOENIX_HOOK_" + sync->m_config->nodeID()->hex() + ".txt", std::time(0), sync->m_config->nodeID()->hex(), HookPosition::BEFORE_BLOCK_SYNC, "666666666", "change block header");
                // }
